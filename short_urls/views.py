...
def make_short(request):
    if request.method == 'POST':
        ...
        if long_url_exists == False:
            ...
            r.set(new_url_obj.short_url, long_url)
        ...
        #If long_url already exists, respond with an appropriate message, and do nothing
            ...
            short_url_obj.save()
            long_url = r.get(short_url_obj.short_url)
            if long_url != short_url_obj.long_url:
                long_url = short_url_obj.long_url
                r.set(short_url_obj.short_url, long_url)
            response = {'message': f'long_url {long_url} already exists in database...updated entry\'s time_stamp'}
